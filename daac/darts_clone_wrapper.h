#ifndef __DAAC_DARTS_CLONE_WRAPPER_H__
#define __DAAC_DARTS_CLONE_WRAPPER_H__
...
//
// Copied from
// https://github.com/tensorflow/text/blob/master/tensorflow_text/core/kernels/darts_clone_trie_wrapper.h
class DartsCloneTrieWrapper {
 public:
  ...
  // Returns true if there exists an edge from 'cursor' following 'ch'.
  bool HasTransition(TraversalCursor& cursor, unsigned char ch) const {
    const uint32_t next_node_id = cursor.node_id ^ offset(cursor.unit) ^ ch;
    const uint32_t next_node_unit = trie_array_[next_node_id];
    return label(next_node_unit) == ch;
  }
  ...
  // Returns true if the node pointed by 'cursor' has data.
  bool IsLeaf(const TraversalCursor& cursor) const {
    return has_leaf(cursor.unit);
  }
...
#endif  // __DAAC_DARTS_CLONE_WRAPPER_H__
